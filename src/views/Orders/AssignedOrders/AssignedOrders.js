...
function PendingOrders() {
  ...
  function getOrders() {
    OrderService.getAssigned().then(res => {
      setOrders({
        isLoading: false,
  ...
  function handleSubmit(values, { setErrors }) {
    ...
    OrderService.getAssignedDate(values).then(res => {
      setOrders({
        isLoading: false,
        data: res.data,
        error: null
      ...
      <Box >
        ...
        <MaterialTable
          ...
          columns={[
            ...
            // { title: 'Assigned to', field: '', render: order => _.capitalize(order.assigned_user_data.full_name ? order.assigned_user_data.full_name : null) },
